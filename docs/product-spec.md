...
5. Sent provides WhatsApp-first OTP through the isolated issuer. When WhatsApp delivery fails, the participant may explicitly request SMS. After verification, the issuer adds a browser-created Semaphore identity commitment to a 30-day membership group and returns a signed public group snapshot.
