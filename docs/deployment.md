...
`docker compose up --build` starts web, civic API, a local-only issuer, writing assistant, notification service, moderation service, and four separate PostgreSQL databases. Copy `.env.example` only as a reference; replace every development secret before any shared environment.
...
The local administrator is at `http://localhost:4003` and uses the development `ADMIN_API_KEY`. This local control must be replaced by passkey/MFA authentication before a sensitive pilot.

The local OTP adapter fails closed unless `ALLOW_DEVELOPMENT_PARTICIPATION=true` is set explicitly. Shared staging and production environments must use `OTP_PROVIDER=sentdm`, omit that flag, and configure the Semaphore membership signing keys in their separate trust domains.

## Experimental public beta

The beta uses a Railway project in a European region with separately credentialed services and databases. It displays its experimental status and does not claim citizenship, residence, person-level uniqueness, or representativeness.
